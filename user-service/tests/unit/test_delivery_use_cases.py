...
async def test_valid_add_tg(
    ...
    res = await uc.execute(user_id=user_data.id, delivery_method=delivery_method_tg_add)

    assert res.user_id == user_data.id
...
async def test_invalid_code_add_tg(
    ...
    with pytest.raises(BadRequestException):
        await uc.execute(user_id=user_data.id, delivery_method=delivery_method_tg_add)
