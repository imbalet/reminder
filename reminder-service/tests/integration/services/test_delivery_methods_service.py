...
async def test_create(delivery_methods_service, delivery_method_tg):
...
async def test_get(delivery_methods_service, sample_db_delivery_method_telegram):
...
async def test_get_not_exists(delivery_methods_service):
    res = await delivery_methods_service.get(uuid4())
    assert res is None


async def test_get_by_reminder_id(
    delivery_methods_service,
    sample_db_reminder,
...
async def test_get_by_reminder_id_empty(delivery_methods_service):
    res = await delivery_methods_service.get_by_reminder_id(uuid4())
    assert len(res) == 0


async def test_delete(delivery_methods_service, sample_db_delivery_method_telegram):
...
async def test_delete_not_exists(delivery_methods_service):
    method_id = uuid4()
