...
- [Работа с RabbitMQ](#работа-с-rabbitmq)
  - [Напоминания, при отправке которых произошла ошибка z](#напоминания-при-отправке-которых-произошла-ошибка-z)
- [TODO](#todo)
...
```bash
uvicorn reminder_service.main:app --port 8000
```

Или через UV

```bash
uv run -m uvicorn reminder_service.main:app --port 8000
```
...
```bash
docker compose -p app-reminder-service up --build -d
```
...
```bash
docker compose -p app-reminder-service down
```
...
```bash
docker compose -f docker-compose.test.yml -p reminder-test up --build --abort-on-container-exit --exit-code-from app
docker compose -f docker-compose.test.yml -p reminder-test down -v --rmi local
```
...
### Напоминания, при отправке которых произошла ошибка <!-- omit from toc -->z

При ошибке отправки напоминания автоматически помещаются в DLX (dead letter exchange) обменник `RMQ_DLX_FAILED_REMINDERS_NAME` типа `fanout` и доставляются в reminder service и в user service для создания системного уведомления об ошибке отправки.

Reminder service работает с очередью `RMQ_DLQ_FAILED_REMINDERS_NAME`.
...
При добавлении метода доставки user service публикует сообщение с данными метода доставки в очередь `RMQ_DELIVERY_METHOD_ADD_QUEUE`. 
...
При удалении метода доставки публикуется текстовое сообщение с UUID удалённого метода в очередь `RMQ_DELIVERY_METHOD_REMOVE_QUEUE`. 
При этом метод доставки удаляется из базы данных и из всех связанных с ним напоминаниях.
...
Reminder service каждую минуту получает из БД напоминания, которые должны быть отправлены. Далее все напоминания отправляются в очередь `RMQ_REMINDERS_QUEUE` для дальнейшей обработки и отправки пользователю.
